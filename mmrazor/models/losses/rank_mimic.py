...
class RankMimicLoss(nn.Module):
    ...
    def forward(self, 
                student_assgin_results, 
                student_inside_flags,
                student_cls_scores, 
                teacher_assgin_results, 
                teacher_inside_flags,
                teacher_cls_scores):
        """Forward computation.
    ...
    def loss_single(self,
                    student_assign_result,
                    student_inside_flag,
                    student_cls_score,
                    teacher_assign_result,
                    teacher_inside_flag,
                    teacher_cls_score):
        ...
        student_unmap_gt_inds = unmap(student_gt_inds,student_num_anchors,student_inside_flag, fill=0)
        teacher_unmap_gt_inds = unmap(teacher_gt_inds,teacher_num_anchors,teacher_inside_flag, fill=0)

        student_unmap_labels = unmap(student_labels, student_num_anchors, student_inside_flag, fill=-1)
        teacher_unmap_labels = unmap(teacher_labels, teacher_num_anchors, teacher_inside_flag, fill=-1)
        loss = 0
        for i in range(1, student_num_gts + 1):
            ...
            student_anchor_scores = student_cls_score[student_score_inds, student_label_inds]
            teacher_anchor_scores = teacher_cls_score[teacher_score_inds, teacher_label_inds]
