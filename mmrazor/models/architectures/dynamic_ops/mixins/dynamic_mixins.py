...
class DynamicSequentialMixin(DynamicMixin):
    """A mixin class for Pytorch Sequential, which can mutate ``depth``."""
    ...
    def _register_mutable_depth(self, mutable_depth):
        assert hasattr(self, 'mutable_attrs')
        max_depth = mutable_depth.max_choice
        if max_depth > len(self):
            raise ValueError('Expect max size of mutable to be less or equal'
                             f' than {len(self)}, but got: {max_depth}.')

        self.mutable_attrs['depth'] = mutable_depth

    def to_static_op(self: nn.Sequential) -> nn.Module:
        """Convert dynamic Sequential to :obj:`torch.nn.Sequential`.

        Returns:
            torch.nn.Sequential: :obj:`torch.nn.Sequential` with fixed
                depth.
        """
        self.check_if_mutables_fixed()

        if 'depth' in self.mutable_attrs:
            fixed_depth = self.mutable_attrs['depth'].current_choice
        else:
    ...
    def get_dynamic_depth(self):
        if 'depth' in self.mutable_attrs:
            current_depth = self.mutable_attrs['depth'].current_choice
        else:
            current_depth = len(self)
