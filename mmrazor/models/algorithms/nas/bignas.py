...
class BigNAS(BaseAlgorithm):
    ...
    def set_attentivenas_subnet(self, scale='a0') -> None:
        """Set subnet."""
        setting = {
            'a_max': {
                'resolution': 288,
                'width': [24, 24, 32, 40, 72, 128, 216, 224, 1984],
        ...
        for name, mutator in self.mutators.items():
            if name == 'channel_mutator':
                channel = dict()
                width = setting[scale]['width']
                for i in range(len(width)):
                    channel.update({i: width[i]})
                mutator.set_choices(channel)
            elif name == 'value_mutator':
                value = dict()
                kernel_size = setting[scale]['kernel_size']
                depth = setting[scale]['depth']
                expand_ratio = setting[scale]['expand_ratio']
                resolution = [
                    setting[scale]['resolution'], setting[scale]['resolution']
                ]

                # according to search_group
                value[82] = resolution
                kernel_loc = [0, 5, 13, 28, 37, 56, 75]
                depth_loc = [4, 12, 27, 36, 55, 74, 81]
                expand_ratio_loc = [
                    range(1, 4),
                    range(6, 12),
                    range(14, 27),
                ...
                for i in range(len(kernel_size)):
                    value.update({kernel_loc[i]: kernel_size[i]})
                for i in range(len(depth)):
                    value.update({depth_loc[i]: depth[i]})
                for i in range(len(expand_ratio)):
                    range_ = expand_ratio_loc[i]
                    for j in range_:
                        value.update({j: expand_ratio[i]})

                mutator.set_choices(value)
