...
def build_graphmodule(model: nn.Module,
    ...
    Args:
        ...
        fx_graph (Graph): Traced fx graph. ``fx_graph`` contains the nodes
            this GraphModule should use for code generation.
        name (str): ``name`` denotes the name of this GraphModule for debugging
            purposes. If it's unset, all error messages will report as
            originating from ``GraphModule``. It may be helpful to set this
            to ``root``'s original name or a name that makes sense within the
            context of your transform.
    """
