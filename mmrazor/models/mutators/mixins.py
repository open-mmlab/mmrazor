...
def _build_name_mutable_mapping(mutable_class_type: Type[BaseMutable],
                                supernet: Module) -> Dict[str, BaseMutable]:
    """Mapping module name to mutable."""
    name2mutable: Dict[str, BaseMutable] = dict()
    for name, module in supernet.named_modules():
        if isinstance(module, mutable_class_type):
            name2mutable[name] = module
    return name2mutable


def _build_alias_names_mapping(mutable_class_type: Type[BaseMutable],
...
def _check_valid_groups(alias2mutable_names: Dict[str, List[str]],
...
class GroupMixin:
    ...
    # TODO
    # optimize group logic
    def group_by_name_and_alias(self: MutatorProtocol, supernet: Module,
                                custom_group: List[List[str]]) -> Dict:
        ...
        Args:
            supernet (:obj:`torch.nn.Module`): The supernet to be searched
                in your algorithm.
        """
        name2mutable = _build_name_mutable_mapping(self.mutable_class_type,
                                                   supernet)
        alias2mutable_names = _build_alias_names_mapping(
            self.mutable_class_type, supernet)

        # Check whether the custom group is valid
        if len(custom_group) > 0:
            _check_valid_groups(alias2mutable_names, name2mutable,
                                custom_group)
