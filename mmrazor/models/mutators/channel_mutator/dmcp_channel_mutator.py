...
class DMCPChannelMutator(ChannelMutator[DMCPChannelUnit]):
    ...
    def prepare_arch_params(self, supernet):
        ...
        for group_id, module in self.search_groups.items():
            ...
            for m in module[0].output_related:
                self._op_arch_align[str(m.name)] = str(group_id)
        ...
        for name, module in supernet.named_modules():
            ...
            # prepare for calulate FLOPs with _traceable_choice
            if name in self._op_arch_align.keys():
                module._traceable_choice = \
                    self.search_groups[int(self._op_arch_align[str(name)])][0]._traceable_choice


    def _generate_arch_message(self, out_channels):
        """
    ...
    def sample_subnet(self, mode, arch_train):
        choices = dict()
    ...
    def set_choices(self, choices: Dict[int, Any]) -> None:
        ...
        for group_id, modules in self.search_groups.items():
            ...
            for module in modules:
                ...
                module._traceable_choice.choice = choice
