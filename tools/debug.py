...
warnings.filterwarnings(
    action='ignore', category=DeprecationWarning, module=r'.*')
...
def prepare_data_loaders(data_path, batch_size=4):
    ...
    trainset = torchvision.datasets.CIFAR10(
        data_path, train=True, transform=transform)
...
print('[float model] Evaluation accuracy on test dataset: %2.2f, %2.2f' %
      (top1.avg, top5.avg))
top1, top5 = evaluate(quantized_model, criterion, data_loader_test)
print(
    '[before serilaization] Evaluation accuracy on test dataset: %2.2f, %2.2f'
    % (top1.avg, top5.avg))

fx_graph_mode_model_file_path = os.path.join(
    saved_model_dir, 'resnet18_fx_graph_mode_quantized.pth')
...
loaded_quantized_model.load_state_dict(
    torch.load(fx_graph_mode_model_file_path))
top1, top5 = evaluate(loaded_quantized_model, criterion, data_loader_test)
print(
    '[after serilaization] Evaluation accuracy on test dataset: %2.2f, %2.2f' %
    (top1.avg, top5.avg))
