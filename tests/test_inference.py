...
def test_init_mmcls_model():
    ...
    model = init_mmcls_model(config, device='cpu', cfg_options={
        'algorithm.mutable_cfg': mutable_file})
...
def test_init_mmdet_model():
    ...
    model = init_mmdet_model(config, device="cpu", cfg_options={
        'algorithm.mutable_cfg': mutable_file})
    assert not hasattr(model, 'architecture')
